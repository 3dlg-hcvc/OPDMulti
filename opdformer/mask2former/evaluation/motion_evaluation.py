...
class MotionEvaluator(DatasetEvaluator):
    ...
    def process(self, inputs, outputs):
        ...
        for input, output in zip(inputs, outputs):
            ...
            # instances = filter_instances(filter_type, type_idx, instances)
            prediction["instances"] = prediction_to_json(
                instances, input["image_id"], motionstate=self.motionstate)
            self._predictions.append(prediction)

    def load_from_file(self, inference_file_path, filter_type, type_idx):
        with PathManager.open(inference_file_path, "rb") as f:
            buffer = io.BytesIO(f.read())
        self._predictions = torch.load(buffer)
        tmp_pred = []
        # get gt for some specific filter operations
        ground_truths = defaultdict(list)
        if filter_type == "part_ratio" or "part_count":
            # get gt for some specific filter operations
            tmp_gt = self._coco_api
            tmp_gt = copy.deepcopy(tmp_gt)
            img_ids = sorted(tmp_gt.getImgIds())
            gts = tmp_gt.loadAnns(tmp_gt.getAnnIds(imgIds=img_ids))
            for gt in gts:
                gt["ignore"] = gt["ignore"] if "ignore" in gt else 0
                gt["ignore"] = "iscrowd" in gt and gt["iscrowd"]
            for gt in gts:
                ground_truths[gt["image_id"], gt["category_id"]].append(gt)
        # tmp code
        # door_num = 0
        # drawer_num = 0
        # lid_num = 0
        ...
        # pdb.set_trace()
        # for gt in gts:
        #     if gt['motion']['part_label'] == "door":
        #         door_num += 1
        #     elif gt['motion']['part_label'] == "drawer":
        #         drawer_num += 1
        #     else:
        #         lid_num += 1
        ...
        for prediction in self._predictions:
            ...
            instances = filter_instances(
                filter_type, type_idx, instances, ground_truths)
            prediction["instances"] = instances
            tmp_pred.append(prediction)
        self._predictions = tmp_pred
    ...
    def evaluate(self):
        ...
        if self._output_dir:
            ...
            # TODO: update the instance and save to another path
    ...
    def _eval_predictions(self, tasks, predictions):
        ...
        if self._output_dir:
            file_path = os.path.join(
                self._output_dir, "coco_motion_results.json")
...
def filter_instances(filter_type, type_idx, instances, ground_truths=None):
    tmp_instances = []
    if filter_type == "all":
        tmp_instances = instances
    elif filter_type == "part_cat":
        for instance in instances:
            if instance['category_id'] != type_idx:
                # instance['score'] = torch.tensor([0.0])
                instance['score'] = 0.0
                # import pdb
                # pdb.set_trace()
    # elif filter_type == "obj_cat":
    #     instances = instances
    elif filter_type == "part_ratio":
        for instance in instances:
            p_area = (instance['bbox'][2]-instance['bbox'][0]) * \
                (instance['bbox'][3]-instance['bbox'][1])
            p_ratio = p_area / (192*256)
            if p_ratio > type_idx:
                instance['score'] = 0.0
    elif filter_type == "part_count":
        # import pdb
        # pdb.set_trace()
        for instance in instances:
            img_id = instance['image_id']
            if len(ground_truths[img_id, 0])+len(ground_truths[img_id, 1])+len(ground_truths[img_id, 2]) != type_idx:
                instance['score'] = 0.0
    elif filter_type == "motion_type":
        for instance in instances:
            if instance['mtype'] != type_idx:
                # instance['score'] = torch.tensor([0.0])
                instance['score'] = 0.0
    return instances
...
def motion_inference_on_dataset(
    model, data_loader, evaluator, inference_file_path=None, filter_file_path=None, filter_type=None, type_idx=None
):
    """
    ...
    if evaluator is None:
        ...
        assert ValueError(
            "Motion Error: the evaluator is not corretly defined")
    evaluator.reset()
    ...
    if inference_file_path == None:
        ...
        logger.info("Loading from existed INFERENCE FILE!!!!!!!!!!!!!!!!!!!")
        evaluator.load_from_file(inference_file_path, filter_type, type_idx)
