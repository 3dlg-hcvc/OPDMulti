...
class MotionCocoEval:
    ...
    def __init__(self, cocoGt=None, cocoDt=None, iouType="segm", motionnet_type="BMCC", MODELATTRPATH=None, TYPE_MATCH=False, PART_CAT=False, MICRO_AVG=False, MACRO_TYPE=False, AxisThres=10, OriginThres=0.25, motionstate=False, image_state_path=None, logger=None):
        """
    ...
    def getDiagLength(self):
        ...
        for model in model_bound:
            diagonal_length[model] = model_bound[model]["diagonal"]
    ...
    def evaluate(self):
        ...
        """
        tic = time.time()
        self.logger.info("Running per image Motion evaluation...")
        p = self.params
        ...
        if not len(ious) == 0:
            for tind, t in enumerate(p.iouThrs):
                for dind, d in enumerate(dt):
                    ...
                    # The diagonal length is for each part (because different part can correspond to different object)
                    object_key = gt[m]["object_key"]
                    diagonal_length = self.diagonal_length[object_key]
    ...
    def summarize(self):
        ...
        def _summarize(ap=1, iouThr=None, areaRng="all", maxDets=100, cat=-1):
            ...
            # MotionNet: motion type and axis mAP
                ...
                titleStr = "Average Precision (All motion Canonical) Threshold)"
                typeStr = "(mAP_MotionCanonical)"
                # dimension of precision: [TxRxKxAxM]
                s = self.eval["mMWP"]
                # IoU
                if iouThr is not None:
                ...
                titleStr = "Average Precision (+MACanonical) Threshold)"
                typeStr = "(mAP_+MACanonical)"
                # dimension of precision: [TxRxKxAxM]
                s = self.eval["mTDWP"]
                # IoU
                if iouThr is not None:
                ...
                titleStr = "Average Precision (Object Pose Trans) Threshold)"
                typeStr = "(mAP_PoseTrans)"
                # dimension of precision: [TxRxKxAxM]
                s = self.eval["mETP"]
                # IoU
                if iouThr is not None:
                ...
                titleStr = "Average Precision (Object Pose Rot) Threshold)"
                typeStr = "(mAP_PoseRot)"
                # dimension of precision: [TxRxKxAxM]
                s = self.eval["mERP"]
                # IoU
